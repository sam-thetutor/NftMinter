...
import ExtEvent "./motoko/ext/Event";
...
actor class EXTNFT(init_owner : Principal) = this {
  ...
  //allows you to list an nft on the marketplace
  type ListRequest = {
  ...
  private stable var data_mintersTableState : [(AccountIdentifier, ())] = [];
  ...
  //save the transaction history events
  private stable var transactionEventHistory : [(TokenIndex, [TransactionEvent])] = [];
  ...
  private stable var config_collection_name : Text = "SoZo Posts";
  ...
  var _minters : HashMap.HashMap<AccountIdentifier, ()> = HashMap.fromIter(data_mintersTableState.vals(), 0, AID.equal, AID.hash);
  ...
  var _transactionEventHistory : HashMap.HashMap<TokenIndex, [TransactionEvent]> = HashMap.fromIter(transactionEventHistory.vals(), 0, ExtCore.TokenIndex.equal, ExtCore.TokenIndex.hash);
  ...
  system func preupgrade() {
    ...
    //custom data structures for sozo
    transactionEventHistory := Iter.toArray(_transactionEventHistory.entries());
  ...
  system func postupgrade() {
    ...
    //custom data stuctures for sozo
    transactionEventHistory := [];
  ...
  // CUSTOM functionality for the SOZO marketplace
  // //get token token delegation history
  public query func get_token_transfer_history(tokenid : Nat32) : async [TransactionEvent] {
    switch (_transactionEventHistory.get(tokenid)) {
      case (?data) { return data };
      case (null) { return [] };
    };
  };

  //get all the tokens transfer history
  public query func get_all_transfer_history() : async [(TokenIndex, [TransactionEvent])] {
    return Iter.toArray<(TokenIndex,[TransactionEvent])>(_transactionEventHistory.entries());
  };

  //save the transfer to the marketplace
  func _saveEvent(tokenid : Nat32, tokenIdentifier : TokenIdentifier, from : Principal, to : Text, _type : Text) : () {

    switch (_transactionEventHistory.get(tokenid)) {
      case (?data) {
        let tempBuffer = Buffer.fromArray<TransactionEvent>(data);
        tempBuffer.add({
        ...
        _transactionEventHistory.put(tokenid, Buffer.toArray(tempBuffer));
        return ()

      };
      case (null) {
        _transactionEventHistory.put(tokenid, [{ transaction_type = _type; tokenIdentifier; to; from; transaction_time = Time.now();
         }]);
        return ()
  ...
  public shared (msg) func ext_mint(request : [(AccountIdentifier, Metadata)]) : async [TokenIndex] {
    assert (_isAdmin(msg.caller));
    var ret : [TokenIndex] = [];
    label mints for (r in request.vals()) {
      switch (_minters.get(r.0)) {
        case (?()) {
          break mints;
        };
        case (null) {
          ...
          _saveEvent(data_internalNextTokenId, "", Principal.fromActor(this), r.0, "Mint");
  ...
  public func get_trusted_origins() : async [Text] {
    [
      "http://localhost:5173",
      "http://localhost:5173/",
      "https://sozo-app.pages.dev/",
      "https://sozo-app.pages.dev",
      "https://3lkhz-miaaa-aaaap-qhoma-cai.icp0.io",
      "https://3lkhz-miaaa-aaaap-qhoma-cai.icp0.io/",
      "https://z3cfg-oqaaa-aaaal-ajvgq-cai.icp0.io/",
      "https://z3cfg-oqaaa-aaaal-ajvgq-cai.icp0.io",
    ];
  };

  public func icrc28_trusted_origins() : async { trusted_origins : [Text] } {
    return {
      trusted_origins = [
        "http://localhost:5173",
        "http://localhost:5173/",
  ...
  func _ext_internal_transfer(caller : Principal, request : TransferRequest) : async TransferResponse {
    ...
    //check whether they have included a transaction type on the request
    let transferType = switch (request.transfer_type) {
      case (?data) { "Sale" };
      case (null) { "Transfer" };
    };
    ...
    switch (_registry.get(token)) {
      case (?token_owner) {
        ...
        if (request.notify) {
              ...
              switch (await notifier.tokenTransferNotification(request.token, request.from, request.amount, request.memo)) {
                case (?balance) {
                  if (balance == 1) {
                    ...
                    _saveEvent(token, request.token, caller, receiver, transferType);
          ...
          _transferTokenToUser(token, receiver);
          _capAddTransfer(token, owner, receiver);
          //_saveEvent(token, request.token, caller, Time.now());
          _saveEvent(token, request.token, caller, receiver, transferType);
